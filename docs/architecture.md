...
- **ADR-011 · 2026-07-30 · Schreibweg der Gate-Aktion aus der serverlosen Anzeige (AC3, ADR-005) = Befehl zum Kopieren.** Owner-Entscheid: Kandidat (a). Die Anzeige zeigt den fertigen Befehl zur PM-Anstoss-Aktion (Thema-ID, Lauf-ID) als Text mit Kopier-Funktion; ausgeführt wird er vom Nutzer im Terminal — „klickbar" degradiert bewusst zu „kopierbar", kein neuer technischer Mechanismus. *Begründung:* passt zum serverlosen, buildlosen Grundprinzip der Anzeige (ADR-007/ADR-010); kein Chromium-only-Pfad (verwirft (b)), keine Rücknahme der Server-Ablehnung aus ADR-010 (verwirft (c)). *Verworfen:* (b) Gate-Intent-Datei via File System Access API (asynchron, Chromium-only); (c) minimaler lokaler HTTP-Endpunkt nur für die Gate-Aktion (zweite Laufzeit, widerspricht ADR-010). *Konsequenz:* „PM anstossen" in der UI zeigt den Befehl inkl. Thema-/Lauf-Kontext und einen Kopieren-Button; kein Auto-Submit, keine Bestätigung im Dashboard selbst — der eigentliche Anstoss läuft weiterhin über die bestehende Schicht-1-Schnittstelle (gate-pm-anstoss.md). *Confidence:* hoch. *Reeval-Trigger:* die Anzeige bekommt einen begleitenden lokalen Prozess aus anderem Grund (dann würde (c) neu attraktiv).
