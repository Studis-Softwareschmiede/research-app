...
- **Tastatur-Navigation:** komplette Bedienung ohne Maus — Tabellen-Zeilen als `<button>`/`tabindex`-fähige Elemente, Divergenz-Auswahl über native `<select>`, Gate-Dialog per `Esc` schliessbar (native `<dialog>`-Verhalten).
- **Touch-Targets:** ≥ 44×44px für Gate-Buttons und Zeilen-Klickfläche (übertrifft WCAG 2.2 AA 2.5.8-Minimum von 24px bewusst, da Analyse-Werkzeug auch am Trackpad/Touch bedienbar bleiben soll).
